...
<!-- Copyright © 2005-2010 Jonathan Porter [http://j.porter.name/]
...
'XHTML' refers to the strict version of either 'XHTML™ 1.0 The Extensible
...
'HTML' refers to the strict version in the 'HTML 4.01 Specification'
...
<xsl:stylesheet 
  ...
  <xsl:template match="xhtml:map/@name"/>
  ...
  ensure a valid HTML document is produced.  Semantically the XHTML and HTML
  ...
  <xsl:template match="@xml:space|xhtml:html/@version|xhtml:html/@xsi:schemaLocation"/>

  <!-- <xsl:template name="checkId">
    <xsl:analyze-string select="." regex="^[A-Za-z][A-Za-z0-9:_.-]*$">
      <xsl:non-matching-substring>
        <xsl:message  terminate="yes">The XHTML "id" will not evaluate to a valid HTML "name" or "id".</xsl:message>
      </xsl:non-matching-substring>
    </xsl:analyze-string>
  </xsl:template> -->
